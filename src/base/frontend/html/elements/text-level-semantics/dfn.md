---
title: "<dfn>"
description: "Термин, определяемый в окружающем контексте."
outline: [2, 3]
---
...
Термин, определяемый в окружающем контексте.
...
```html
<p><dfn id="term-dom">DOM</dfn> — объектная модель документа. См. <a href="#term-dom">определение DOM</a>.</p>
```
...
- Не используйте XML-запись `<dfn />` как замену закрывающему тегу в HTML.
...
- [Определение `dfn` в WHATWG](https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-dfn-element)
