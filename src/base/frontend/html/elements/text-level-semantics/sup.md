...
```html
<p>Площадь: 24 м<sup>2</sup>. Утверждение подтверждено источником<sup><a href="#note-1">1</a></sup>.</p>
```
...
- Не используйте XML-запись `<sup />` как замену закрывающему тегу в HTML.
...
- [Определение `sup` в WHATWG](https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-sub-and-sup-elements)
