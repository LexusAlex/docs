---
title: "<s>"
description: "Текст, который больше не точен или не актуален, но не является правкой документа."
outline: [2, 3]
---
...
Текст, который больше не точен или не актуален, но не является правкой документа.
...
```html
<p><s>4 990 ₽</s> <strong>3 990 ₽</strong></p>
```
...
- Не используйте XML-запись `<s />` как замену закрывающему тегу в HTML.
...
- [Определение `s` в WHATWG](https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-s-element)
