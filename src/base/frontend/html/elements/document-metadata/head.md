---
title: "<head>"
description: "Контейнер метаданных документа, не отображаемых как основное содержимое страницы."
outline: [2, 3]
---
...
Контейнер метаданных документа, не отображаемых как основное содержимое страницы.
...
| Содержимое | `metadata content` | metadata content * |
...
- Не используйте XML-запись `<head />` как замену закрывающему тегу в HTML.

## DOM-интерфейс

Элемент представлен интерфейсом [`HTMLHeadElement`](https://html.spec.whatwg.org/multipage/semantics.html#htmlheadelement). Для чтения исходных атрибутов всегда доступны `getAttribute()`, `setAttribute()` и `removeAttribute()`; специализированные IDL-свойства описаны в спецификации интерфейса.

## Спецификация

- [Определение `head` в WHATWG](https://html.spec.whatwg.org/multipage/semantics.html#the-head-element)
