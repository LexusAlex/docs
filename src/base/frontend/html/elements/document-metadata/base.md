---
title: "<base>"
description: "Задаёт базовый URL и базовую цель для относительных ссылок документа."
outline: [2, 3]
---
...
Задаёт базовый URL и базовую цель для относительных ссылок документа.
...
- **Специальные атрибуты:** [`href`](/base/frontend/html/attributes/element-specific/href), [`target`](/base/frontend/html/attributes/element-specific/target).
...
- Не добавляйте закрывающий тег `</base>`.

## DOM-интерфейс

Элемент представлен интерфейсом [`HTMLBaseElement`](https://html.spec.whatwg.org/multipage/semantics.html#htmlbaseelement). Для чтения исходных атрибутов всегда доступны `getAttribute()`, `setAttribute()` и `removeAttribute()`; специализированные IDL-свойства описаны в спецификации интерфейса.

## Спецификация

- [Определение `base` в WHATWG](https://html.spec.whatwg.org/multipage/semantics.html#the-base-element)
