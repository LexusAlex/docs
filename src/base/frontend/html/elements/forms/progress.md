...
- **Специальные атрибуты:** [`value`](/base/frontend/html/attributes/element-specific/value), [`max`](/base/frontend/html/attributes/element-specific/max).
...
```html
<label for="upload">Загрузка</label>
...
- Не используйте XML-запись `<progress />` как замену закрывающему тегу в HTML.

## DOM-интерфейс

Элемент представлен интерфейсом [`HTMLProgressElement`](https://html.spec.whatwg.org/multipage/form-elements.html#htmlprogresselement). Для чтения исходных атрибутов всегда доступны `getAttribute()`, `setAttribute()` и `removeAttribute()`; специализированные IDL-свойства описаны в спецификации интерфейса.

## Спецификация

- [Определение `progress` в WHATWG](https://html.spec.whatwg.org/multipage/form-elements.html#the-progress-element)
