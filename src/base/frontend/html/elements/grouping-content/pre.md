---
title: "<pre>"
description: "Блок предварительно форматированного текста, где пробелы и переносы значимы."
outline: [2, 3]
---
...
Блок предварительно форматированного текста, где пробелы и переносы значимы.
...
- Не используйте XML-запись `<pre />` как замену закрывающему тегу в HTML.

## DOM-интерфейс

Элемент представлен интерфейсом [`HTMLPreElement`](https://html.spec.whatwg.org/multipage/grouping-content.html#htmlpreelement). Для чтения исходных атрибутов всегда доступны `getAttribute()`, `setAttribute()` и `removeAttribute()`; специализированные IDL-свойства описаны в спецификации интерфейса.

## Спецификация

- [Определение `pre` в WHATWG](https://html.spec.whatwg.org/multipage/grouping-content.html#the-pre-element)
