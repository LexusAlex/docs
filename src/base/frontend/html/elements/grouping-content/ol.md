---
title: "<ol>"
description: "Упорядоченный список, для которого порядок элементов имеет значение."
outline: [2, 3]
---
...
Упорядоченный список, для которого порядок элементов имеет значение.
...
- **Специальные атрибуты:** [`reversed`](/base/frontend/html/attributes/element-specific/reversed), [`start`](/base/frontend/html/attributes/element-specific/start), [`type`](/base/frontend/html/attributes/element-specific/type).
...
- Не используйте XML-запись `<ol />` как замену закрывающему тегу в HTML.

## DOM-интерфейс

Элемент представлен интерфейсом [`HTMLOListElement`](https://html.spec.whatwg.org/multipage/grouping-content.html#htmlolistelement). Для чтения исходных атрибутов всегда доступны `getAttribute()`, `setAttribute()` и `removeAttribute()`; специализированные IDL-свойства описаны в спецификации интерфейса.

## Спецификация

- [Определение `ol` в WHATWG](https://html.spec.whatwg.org/multipage/grouping-content.html#the-ol-element)
