---
title: "onbeforematch"
description: "Атрибут-обработчик события `beforematch`. Для прикладного кода обычно предпочтительнее `addEventListener()`."
outline: [2, 3]
---
...
Атрибут-обработчик события `beforematch`. Для прикладного кода обычно предпочтительнее `addEventListener()`.
...
|---|---|---|---|
| Все HTML-элементы | beforematch event handler | Event handler content attribute | [WHATWG](https://html.spec.whatwg.org/multipage/webappapis.html#handler-onbeforematch) |
...
```html
<section id="demo" hidden="until-found" onbeforematch="console.log(event.type)">Найденный фрагмент</section>
```
...
- [Определение для всех HTML-элементов](https://html.spec.whatwg.org/multipage/webappapis.html#handler-onbeforematch)
