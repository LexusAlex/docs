---
title: "onmouseleave"
description: "Атрибут-обработчик события `mouseleave`. Для прикладного кода обычно предпочтительнее `addEventListener()`."
outline: [2, 3]
---
...
Атрибут-обработчик события `mouseleave`. Для прикладного кода обычно предпочтительнее `addEventListener()`.
...
|---|---|---|---|
| Все HTML-элементы | mouseleave event handler | Event handler content attribute | [WHATWG](https://html.spec.whatwg.org/multipage/webappapis.html#handler-onmouseleave) |
...
```html
<button id="demo" onmouseleave="console.log(event.type)">Проверить</button>
```
...
- [Определение для всех HTML-элементов](https://html.spec.whatwg.org/multipage/webappapis.html#handler-onmouseleave)
