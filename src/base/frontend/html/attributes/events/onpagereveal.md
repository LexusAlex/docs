---
title: "onpagereveal"
description: "Атрибут-обработчик события `pagereveal`. Для прикладного кода обычно предпочтительнее `addEventListener()`."
outline: [2, 3]
---
...
Атрибут-обработчик события `pagereveal`. Для прикладного кода обычно предпочтительнее `addEventListener()`.
...
|---|---|---|---|
| [`body`](/base/frontend/html/elements/sections/body) | pagereveal event handler for Window object | Event handler content attribute | [WHATWG](https://html.spec.whatwg.org/multipage/webappapis.html#handler-window-onpagereveal) |
...
```html
<body onpagereveal="console.log(event.type)">Содержимое страницы</body>
```
...
- [Определение `<body>`](https://html.spec.whatwg.org/multipage/webappapis.html#handler-window-onpagereveal)
