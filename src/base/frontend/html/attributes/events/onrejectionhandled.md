...
description: "Атрибут-обработчик события `rejectionhandled`. Для прикладного кода обычно предпочтительнее `addEventListener()`."
outline: [2, 3]
---
...
Атрибут-обработчик события `rejectionhandled`. Для прикладного кода обычно предпочтительнее `addEventListener()`.
...
|---|---|---|---|
| [`body`](/base/frontend/html/elements/sections/body) | rejectionhandled event handler for Window object | Event handler content attribute | [WHATWG](https://html.spec.whatwg.org/multipage/webappapis.html#handler-window-onrejectionhandled) |
...
```html
<body onrejectionhandled="console.log(event.type)">Содержимое страницы</body>
```
...
- [Определение `<body>`](https://html.spec.whatwg.org/multipage/webappapis.html#handler-window-onrejectionhandled)
