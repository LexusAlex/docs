---
title: "oncontextrestored"
description: "Атрибут-обработчик события `contextrestored`. Для прикладного кода обычно предпочтительнее `addEventListener()`."
outline: [2, 3]
---
...
Атрибут-обработчик события `contextrestored`. Для прикладного кода обычно предпочтительнее `addEventListener()`.
...
|---|---|---|---|
| Все HTML-элементы | contextrestored event handler | Event handler content attribute | [WHATWG](https://html.spec.whatwg.org/multipage/webappapis.html#handler-oncontextrestored) |
...
```html
<canvas id="demo" width="320" height="180" oncontextrestored="console.log(event.type)"></canvas>
```
...
element.addEventListener('contextrestored', (event) => {
...
- [Определение для всех HTML-элементов](https://html.spec.whatwg.org/multipage/webappapis.html#handler-oncontextrestored)
