---
title: "onload"
description: "Атрибут-обработчик события `load`. Для прикладного кода обычно предпочтительнее `addEventListener()`."
outline: [2, 3]
---
...
Атрибут-обработчик события `load`. Для прикладного кода обычно предпочтительнее `addEventListener()`.
...
|---|---|---|---|
| Все HTML-элементы | load event handler | Event handler content attribute | [WHATWG](https://html.spec.whatwg.org/multipage/webappapis.html#handler-onload) |
...
```html
<img id="demo" src="photo.jpg" alt="Горная долина" onload="console.log(event.type)">
```
...
- [Определение для всех HTML-элементов](https://html.spec.whatwg.org/multipage/webappapis.html#handler-onload)
