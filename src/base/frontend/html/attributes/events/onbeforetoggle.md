...
description: "Атрибут-обработчик события `beforetoggle`. Для прикладного кода обычно предпочтительнее `addEventListener()`."
outline: [2, 3]
---
...
Атрибут-обработчик события `beforetoggle`. Для прикладного кода обычно предпочтительнее `addEventListener()`.
...
|---|---|---|---|
| Все HTML-элементы | beforetoggle event handler | Event handler content attribute | [WHATWG](https://html.spec.whatwg.org/multipage/webappapis.html#handler-onbeforetoggle) |
...
```html
<button popovertarget="demo">Открыть</button>
<div id="demo" popover onbeforetoggle="console.log(event.newState)">Подсказка</div>
```
...
- [Определение для всех HTML-элементов](https://html.spec.whatwg.org/multipage/webappapis.html#handler-onbeforetoggle)
