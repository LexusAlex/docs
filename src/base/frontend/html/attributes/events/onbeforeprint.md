---
title: "onbeforeprint"
description: "Атрибут-обработчик события `beforeprint`. Для прикладного кода обычно предпочтительнее `addEventListener()`."
outline: [2, 3]
---
...
Атрибут-обработчик события `beforeprint`. Для прикладного кода обычно предпочтительнее `addEventListener()`.
...
|---|---|---|---|
| [`body`](/base/frontend/html/elements/sections/body) | beforeprint event handler for Window object | Event handler content attribute | [WHATWG](https://html.spec.whatwg.org/multipage/webappapis.html#handler-window-onbeforeprint) |
...
```html
<body onbeforeprint="console.log(event.type)">Содержимое страницы</body>
```
...
- [Определение `<body>`](https://html.spec.whatwg.org/multipage/webappapis.html#handler-window-onbeforeprint)
