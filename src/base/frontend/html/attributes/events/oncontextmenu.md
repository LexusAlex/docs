---
title: "oncontextmenu"
description: "Атрибут-обработчик события `contextmenu`. Для прикладного кода обычно предпочтительнее `addEventListener()`."
outline: [2, 3]
---
...
Атрибут-обработчик события `contextmenu`. Для прикладного кода обычно предпочтительнее `addEventListener()`.
...
|---|---|---|---|
| Все HTML-элементы | contextmenu event handler | Event handler content attribute | [WHATWG](https://html.spec.whatwg.org/multipage/webappapis.html#handler-oncontextmenu) |
...
```html
<button id="demo" oncontextmenu="console.log(event.type)">Проверить</button>
```
...
- [Определение для всех HTML-элементов](https://html.spec.whatwg.org/multipage/webappapis.html#handler-oncontextmenu)
