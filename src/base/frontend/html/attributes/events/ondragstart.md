---
title: "ondragstart"
description: "Атрибут-обработчик события `dragstart`. Для прикладного кода обычно предпочтительнее `addEventListener()`."
outline: [2, 3]
---
...
Атрибут-обработчик события `dragstart`. Для прикладного кода обычно предпочтительнее `addEventListener()`.
...
|---|---|---|---|
| Все HTML-элементы | dragstart event handler | Event handler content attribute | [WHATWG](https://html.spec.whatwg.org/multipage/webappapis.html#handler-ondragstart) |
...
```html
<div id="demo" draggable="true" ondragstart="console.log(event.type)">Перетащите блок</div>
```
...
- [Определение для всех HTML-элементов](https://html.spec.whatwg.org/multipage/webappapis.html#handler-ondragstart)
