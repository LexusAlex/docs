---
title: "onsubmit"
description: "Атрибут-обработчик события `submit`. Для прикладного кода обычно предпочтительнее `addEventListener()`."
outline: [2, 3]
---
...
Атрибут-обработчик события `submit`. Для прикладного кода обычно предпочтительнее `addEventListener()`.
...
|---|---|---|---|
| Все HTML-элементы | submit event handler | Event handler content attribute | [WHATWG](https://html.spec.whatwg.org/multipage/webappapis.html#handler-onsubmit) |
...
```html
<form id="demo" action="/submit" onsubmit="console.log(event.type)"><button>Отправить</button></form>
```
...
- [Определение для всех HTML-элементов](https://html.spec.whatwg.org/multipage/webappapis.html#handler-onsubmit)
