---
title: "oncontextlost"
description: "Атрибут-обработчик события `contextlost`. Для прикладного кода обычно предпочтительнее `addEventListener()`."
outline: [2, 3]
---
...
Атрибут-обработчик события `contextlost`. Для прикладного кода обычно предпочтительнее `addEventListener()`.
...
|---|---|---|---|
| Все HTML-элементы | contextlost event handler | Event handler content attribute | [WHATWG](https://html.spec.whatwg.org/multipage/webappapis.html#handler-oncontextlost) |
...
```html
<canvas id="demo" width="320" height="180" oncontextlost="console.log(event.type)"></canvas>
```
...
element.addEventListener('contextlost', (event) => {
...
- [Определение для всех HTML-элементов](https://html.spec.whatwg.org/multipage/webappapis.html#handler-oncontextlost)
