...
description: "Атрибут-обработчик события `securitypolicyviolation`. Для прикладного кода обычно предпочтительнее `addEventListener()`."
outline: [2, 3]
---
...
Атрибут-обработчик события `securitypolicyviolation`. Для прикладного кода обычно предпочтительнее `addEventListener()`.
...
|---|---|---|---|
| Все HTML-элементы | securitypolicyviolation event handler | Event handler content attribute | [WHATWG](https://html.spec.whatwg.org/multipage/webappapis.html#handler-onsecuritypolicyviolation) |
...
```html
<body onsecuritypolicyviolation="console.log(event.blockedURI)">Содержимое страницы</body>
```
...
- [Определение для всех HTML-элементов](https://html.spec.whatwg.org/multipage/webappapis.html#handler-onsecuritypolicyviolation)
