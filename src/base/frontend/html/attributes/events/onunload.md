---
title: "onunload"
description: "Атрибут-обработчик события `unload`. Для прикладного кода обычно предпочтительнее `addEventListener()`."
outline: [2, 3]
---
...
Атрибут-обработчик события `unload`. Для прикладного кода обычно предпочтительнее `addEventListener()`.
...
|---|---|---|---|
| [`body`](/base/frontend/html/elements/sections/body) | unload event handler for Window object | Event handler content attribute | [WHATWG](https://html.spec.whatwg.org/multipage/webappapis.html#handler-window-onunload) |
...
```html
<body onunload="console.log(event.type)">Содержимое страницы</body>
```
...
- [Определение `<body>`](https://html.spec.whatwg.org/multipage/webappapis.html#handler-window-onunload)
