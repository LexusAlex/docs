---
title: "onslotchange"
description: "Атрибут-обработчик события `slotchange`. Для прикладного кода обычно предпочтительнее `addEventListener()`."
outline: [2, 3]
---
...
Атрибут-обработчик события `slotchange`. Для прикладного кода обычно предпочтительнее `addEventListener()`.
...
|---|---|---|---|
| Все HTML-элементы | slotchange event handler | Event handler content attribute | [WHATWG](https://html.spec.whatwg.org/multipage/webappapis.html#handler-onslotchange) |
...
<article-card>
  ...
  <template shadowrootmode="open">
    <slot id="demo" name="title" onslotchange="console.log(event.type)"></slot>
  </template>
</article-card>
```
...
- [Определение для всех HTML-элементов](https://html.spec.whatwg.org/multipage/webappapis.html#handler-onslotchange)
