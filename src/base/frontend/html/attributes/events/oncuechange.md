---
title: "oncuechange"
description: "Атрибут-обработчик события `cuechange`. Для прикладного кода обычно предпочтительнее `addEventListener()`."
outline: [2, 3]
---
...
Атрибут-обработчик события `cuechange`. Для прикладного кода обычно предпочтительнее `addEventListener()`.
...
|---|---|---|---|
| Все HTML-элементы | cuechange event handler | Event handler content attribute | [WHATWG](https://html.spec.whatwg.org/multipage/webappapis.html#handler-oncuechange) |
...
```html
<video controls>
  <track id="demo" default kind="captions" src="captions-ru.vtt" srclang="ru" oncuechange="console.log(event.type)">
</video>
```
...
element.addEventListener('cuechange', (event) => {
...
- [Определение для всех HTML-элементов](https://html.spec.whatwg.org/multipage/webappapis.html#handler-oncuechange)
