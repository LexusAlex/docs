---
title: "onpopstate"
description: "Атрибут-обработчик события `popstate`. Для прикладного кода обычно предпочтительнее `addEventListener()`."
outline: [2, 3]
---
...
Атрибут-обработчик события `popstate`. Для прикладного кода обычно предпочтительнее `addEventListener()`.
...
|---|---|---|---|
| [`body`](/base/frontend/html/elements/sections/body) | popstate event handler for Window object | Event handler content attribute | [WHATWG](https://html.spec.whatwg.org/multipage/webappapis.html#handler-window-onpopstate) |
...
```html
<body onpopstate="console.log(event.type)">Содержимое страницы</body>
```
...
- [Определение `<body>`](https://html.spec.whatwg.org/multipage/webappapis.html#handler-window-onpopstate)
