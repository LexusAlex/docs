---
title: "ondragenter"
description: "Атрибут-обработчик события `dragenter`. Для прикладного кода обычно предпочтительнее `addEventListener()`."
outline: [2, 3]
---
...
Атрибут-обработчик события `dragenter`. Для прикладного кода обычно предпочтительнее `addEventListener()`.
...
|---|---|---|---|
| Все HTML-элементы | dragenter event handler | Event handler content attribute | [WHATWG](https://html.spec.whatwg.org/multipage/webappapis.html#handler-ondragenter) |
...
```html
<div id="demo" draggable="true" ondragenter="console.log(event.type)">Перетащите блок</div>
```
...
- [Определение для всех HTML-элементов](https://html.spec.whatwg.org/multipage/webappapis.html#handler-ondragenter)
