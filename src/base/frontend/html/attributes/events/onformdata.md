---
title: "onformdata"
description: "Атрибут-обработчик события `formdata`. Для прикладного кода обычно предпочтительнее `addEventListener()`."
outline: [2, 3]
---
...
Атрибут-обработчик события `formdata`. Для прикладного кода обычно предпочтительнее `addEventListener()`.
...
|---|---|---|---|
| Все HTML-элементы | formdata event handler | Event handler content attribute | [WHATWG](https://html.spec.whatwg.org/multipage/webappapis.html#handler-onformdata) |
...
```html
<form id="demo" action="/submit" onformdata="console.log(event.type)"><button>Отправить</button></form>
```
...
- [Определение для всех HTML-элементов](https://html.spec.whatwg.org/multipage/webappapis.html#handler-onformdata)
