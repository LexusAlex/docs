---
title: "ondblclick"
description: "Атрибут-обработчик события `dblclick`. Для прикладного кода обычно предпочтительнее `addEventListener()`."
outline: [2, 3]
---
...
Атрибут-обработчик события `dblclick`. Для прикладного кода обычно предпочтительнее `addEventListener()`.
...
|---|---|---|---|
| Все HTML-элементы | dblclick event handler | Event handler content attribute | [WHATWG](https://html.spec.whatwg.org/multipage/webappapis.html#handler-ondblclick) |
...
```html
<button id="demo" ondblclick="console.log(event.type)">Проверить</button>
```
...
- [Определение для всех HTML-элементов](https://html.spec.whatwg.org/multipage/webappapis.html#handler-ondblclick)
